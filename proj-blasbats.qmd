...
climate %>% 
  ...
  filter(scenario %in% c("Current", "SSP5-8.5")) %>% 
  ...
  theme(legend.position = "bottom", axis.title = element_blank()) +
  ggtitle("BIO12 (Annual Precipitation)") +
  scale_fill_manual(guide = "none", values = c("NA", "yellow"))
```
...
all <- new_pars %>% 
  ...
  mutate(log_climate_effect = 
           ...
           delta4*bio15*wsg +
           delta5*bio4*wsg) %>% 
  select(-alpha, -beta, -starts_with("gamma"), -starts_with("delta")) %>% 
  gather(climate_variable, climate_value, -iter, -clus, -type, -model, -scenario,
         -period, -wsg_quantile, -wsg, -log_climate_effect)
...
fig_dat <- vroom::vroom(file.path(path, "all.tsv")) %>% 
  filter(wsg_quantile == 0.5) %>% 
  group_by(iter, clus, type, model, scenario, period) %>% 
  summarise(log_climate_effect = mean(log_climate_effect))
